import { Notification, sequelize, User } from "~/models"
...
export const READ_NOTI = async (ws: WebSocket, data: WSMessage, info: User) => {
    const dataReq: DataNoti = data.payload;
    const readTransaction = await sequelize.transaction();
    
    const isRead = await isReadNotiLive(info.id, dataReq.noti_id, readTransaction);
